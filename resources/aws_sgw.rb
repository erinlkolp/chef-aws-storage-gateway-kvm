...
action :install do
  ...
  remote_file "#{target_directory}/govcloud-#{aws_dl_filename}" do
    source "#{govcloud_dl_location}/#{aws_dl_filename}"
    owner 'root'
    group 'root'
    mode '0770'
    show_progress true
  ...
  execute 'Extract AWS GovCloud VM Template' do
    command "unzip -o govcloud-#{aws_dl_filename} -d golden-images/"
    cwd "#{target_directory}"
    action :run
    creates "#{target_directory}/golden-images/prod-us-gov-west-1/prod-us-gov-west-1.qcow2"
  end
...
action :create do
  ...
  when "govcloud"
    file "#{target_directory}/sgw-disks/#{new_resource.name}-primary-disk.qcow2" do
      ...
      group 'qemu'
      mode '0770'
      content ::File.open("#{target_directory}/golden-images/prod-us-gov-west-1/prod-us-gov-west-1.qcow2").read
      action :create
  ...
  when "commercial"
    file "#{target_directory}/sgw-disks/#{new_resource.name}-primary-disk.qcow2" do
      ...
      group 'qemu'
      mode '0770'
      content ::File.open("#{target_directory}/golden-images/prod-us-east-1/prod-us-east-1.qcow2").read
      action :create
      not_if { ::File.exist?("#{target_directory}/sgw-disks/#{new_resource.name}-primary-disk.qcow2") }
    end
  end
